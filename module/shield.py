...
class Shield:
    def __init__(self, player):
        ...
        # ⚡ Bolt Optimization: Pre-scale and cache the shield image
        # to avoid expensive QPixmap.scaled() calls every frame in draw()
        ...
        if not self.image.isNull():
            ...
            if not hasattr(self.player.game_widget, 'health_system') or \
               self.player.game_widget.health_system.is_visible:
                ...
                # ⚡ Bolt Optimization: Use the pre-scaled image instead of scaling on the fly
