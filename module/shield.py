...
class Shield:
    def __init__(self, player):
        ...
        # ⚡ Bolt: Pre-scale shield image to avoid expensive scaling per-frame in draw loop
        self.scaled_image = None
        if not self.image.isNull():
            self.scaled_image = self.image.scaled(self.width, self.height, Qt.KeepAspectRatio, Qt.FastTransformation)
    ...
    def draw(self, painter):
        if self.scaled_image and not self.scaled_image.isNull():
            ...
            if not hasattr(self.player.game_widget, 'health_system') or \
               self.player.game_widget.health_system.is_visible:
                ...
                # ⚡ Bolt: Use pre-scaled image for optimized rendering
