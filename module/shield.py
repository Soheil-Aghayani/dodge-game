...
class Shield:
    def __init__(self, player):
        ...
        # ⚡ Bolt: Pre-scale the shield image during initialization
        if not self.image.isNull():
            self.scaled_shield = self.image.scaled(
                self.width, self.height,
                Qt.KeepAspectRatio, Qt.FastTransformation
            )
        else:
            self.scaled_shield = None
        ...
        if self.scaled_shield:
            ...
            if not hasattr(self.player.game_widget, 'health_system') or \
               self.player.game_widget.health_system.is_visible:
                ...
                # ⚡ Bolt: Use pre-scaled image instead of scaling on the fly
