...
class Shield:
    def __init__(self, player):
        ...
        # Performance Optimization: Pre-scale image to avoid expensive QPixmap.scaled()
        # calls on every frame during the render loop (draw method).
        # Impact: Saves ~180 pixmap scale re-allocations per shield instance (3s at 60fps)
    ...
    def draw(self, painter):
        if getattr(self, 'scaled_image', None) is not None:
