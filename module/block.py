...
class Block:
    ...
    def check_collision(self, player_bounds):
        # player_bounds is a pre-calculated tuple of (x, y, right, bottom)
        px, py, player_right, player_bottom = player_bounds
        ...
        if self.y + 150 < py or self.y - 150 > player_bottom or self.x + 150 < px or self.x - 150 > player_right:
            return False
        ...
        if (self.explosion and not self.explosion.is_finished) or not self.is_exploding:
            return not (px >= bx + bw or player_right <= bx or py >= by + bh or player_bottom <= by)
        return False