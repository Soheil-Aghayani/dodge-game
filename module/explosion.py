...
class ExplosionAnimation(Animation):
    def __init__(self, x, y, size):
        ...
        # Pre-scale frames for better performance
        # Explosion is 2.5x larger than the block
        self.scaled_size = int(self.size * 2.5)
        self.scaled_frames = []
        for frame in self.frames:
            if frame and not frame.isNull():
                scaled = frame.scaled(self.scaled_size, self.scaled_size, Qt.KeepAspectRatio, Qt.FastTransformation)
