...
class ExplosionAnimation(Animation):
    def __init__(self, x, y, size):
        ...
        # Performance Optimization: Pre-scale explosion frames to avoid expensive
        # QPixmap.scaled() calls on every frame during the render loop.
        # Impact: Saves ~8 pixmap scale re-allocations per explosion animation.
        self.scaled_frames = []
        scaled_size = int(self.size * 2.5)
        for frame in self.frames:
            if frame and not frame.isNull():
                self.scaled_frames.append(frame.scaled(scaled_size, scaled_size,
