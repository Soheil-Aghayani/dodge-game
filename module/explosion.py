...
class ExplosionAnimation(Animation):
    def __init__(self, x, y, size):
        ...
        # Pre-scale explosion frames to improve render performance
        scaled_size = int(self.size * 2.5)
        self.scaled_frames = []
        for f in self.frames:
            if f:
                self.scaled_frames.append(f.scaled(scaled_size, scaled_size, Qt.KeepAspectRatio, Qt.FastTransformation))
            else:
