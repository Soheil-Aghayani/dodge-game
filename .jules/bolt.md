...
## 2026-05-20 - [Bypass redundant geometry math in Block.draw() and pass loop-invariant bounds to child updates]
**Learning:** In PyQt game loops, `Block.draw()` executed heavy geometry math (like `min()`, float divisions, multiplications, int-casts, and divisions) every frame for barrels but immediately discarded them since barrels draw from pre-cached rotated frames. This is a severe bottleneck when drawing thousands of entities per second. Furthermore, child update methods like `Player.update()` were repeatedly fetching properties like `self.game_widget.width()` every frame, causing unnecessary C++ boundary crossings.
**Action:** Push calculations into their respective blocks (e.g. into the `else` block for non-cached images) so they are only calculated when actually needed. Replace implicit float conversions (like `rot_w / 2`) with floor division (`rot_w // 2`). Pass loop-invariant bounds (like `game_width`) down to child update methods as arguments to bypass C++ property fetching.
