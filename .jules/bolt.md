...
## 2026-05-18 - [Pre-calculate Entity Geometries for Hot Loops]\n**Learning:** In PyQt game loops, avoid calculating entity geometries (like hitbox widths, height, offsets, or explosion sizes with float math, multiplication, and integer division) inside hot path methods like `get_rect()` or `draw()`. Repeated math operations inside highly invoked methods limit rendering and collision performance.\n**Action:** Pre-calculate and store these dimensions as instance attributes in `__init__` (e.g. `self.hitbox_x_offset`, `self.explosion_size`) to immediately return simple tuples in `get_rect()` and avoid recalculation.
